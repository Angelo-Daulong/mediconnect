<template>
  ...
  >
    ...
    >
      <template #header> Sign In as Hospital </template>
      <UForm
        :schema="schema"
        :state="state"
        class="space-y-4"
      ...
      <template #footer>
        ...
        <UButton label="Sign In as Admin" @click="navigateTo('/signInAdmin')" />
      </template>
    </UCard>
  </div>
</template>
...
const { userEmail, userRole, hosptalDetails } = storeToRefs(store);
const token = ref('');
...
const onSubmit = async (event: FormSubmitEvent<Schema>) => {
  ...
  const hospital = await $fetch('/api/hospitalUser/login', {
    method: 'POST',
    body: event.data,
    headers: {
  ...
  if (hospital) {
    ...
    userRole.value = 'hospital';
    hosptalDetails.value = hospital;
    // console.log(hospital);
    navigateTo('/hospitalPage');
  } else {
